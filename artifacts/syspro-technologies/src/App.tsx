...
import { ArrowDownRight, ArrowRight, BriefcaseBusiness, Check, Cpu, HardDrive, Linkedin, Mail, MapPin, Menu, Network, Phone, Server, ShieldCheck, UsersRound, X, Wrench, Zap } from 'lucide-react';
...
const services = [
  ...
  { number: '03', title: 'Maintenance Network Accessories', icon: Server, description: 'The switches, racks, cables, and accessories that turn a good network into a dependable one.', tags: ['Spare inventory', 'On-site support'] },
...
function About() {
  return <section id="about" className="scroll-mt-20 bg-[hsl(var(--foreground))] text-[hsl(var(--background))]">
    <div className="mx-auto grid max-w-[1240px] gap-14 px-5 py-24 sm:px-8 lg:grid-cols-[1fr_1fr] lg:gap-24 lg:py-32">
      <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: '-80px' }} variants={fadeUp}><p className="eyebrow text-[hsl(var(--primary))]">About SysPro</p><h2 className="mt-6 max-w-[520px] font-display text-5xl leading-[.95] tracking-[-.06em] sm:text-6xl">Complex systems.<br /><span className="text-[hsl(var(--secondary))]">Human answers.</span></h2><p className="mt-8 max-w-[470px] text-base leading-7 text-[hsl(var(--background)/.65)]">We believe technology support should feel less like a ticket number and more like a trusted person who already understands the stakes. SysPro combines technical discipline with a clear, human way of working.</p><button onClick={() => scrollToId('contact')} className="mt-8 flex items-center gap-3 text-sm font-bold text-[hsl(var(--primary))] hover:text-[hsl(var(--background))]" data-testid="button-about-contact">Start a conversation <ArrowDownRight size={17} /></button></motion.div>
...
function Careers() {
  return <section id="careers" className="scroll-mt-20 border-b border-[hsl(var(--foreground)/.1)] bg-[hsl(var(--secondary)/.2)]">
    <div className="mx-auto max-w-[1240px] px-5 py-24 sm:px-8 lg:py-32">
      <div className="grid gap-10 lg:grid-cols-[.78fr_1.22fr] lg:items-start">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: '-80px' }} variants={fadeUp} className="lg:sticky lg:top-32">
          <p className="eyebrow text-[hsl(var(--accent-foreground))]">Careers at SysPro</p><h2 className="mt-5 max-w-[440px] font-display text-5xl leading-[.95] tracking-[-.06em] sm:text-6xl">Bring your<br /><span className="text-[hsl(var(--accent-foreground))]">signal.</span></h2><p className="mt-7 max-w-[390px] text-base leading-7 text-[hsl(var(--muted-foreground))]">We are always interested in people who care about making technology useful, reliable, and a little easier to live with.</p><div className="mt-7 flex flex-col gap-3 text-sm font-semibold"><span className="flex items-center gap-3"><Check size={16} className="text-[hsl(var(--accent-foreground))]" /> A team that shares context</span><span className="flex items-center gap-3"><Check size={16} className="text-[hsl(var(--accent-foreground))]" /> Work that reaches real people</span><span className="flex items-center gap-3"><Check size={16} className="text-[hsl(var(--accent-foreground))]" /> Room to keep getting better</span></div>
        </motion.div>
        <div className="career-frame overflow-hidden rounded-[1.5rem] shadow-[0_20px_50px_hsl(var(--foreground)/.1)]">
          <div className="flex items-center justify-between border-b border-[hsl(var(--foreground)/.1)] bg-[hsl(var(--card))] px-5 py-4"><div className="flex items-center gap-2 text-sm font-bold"><BriefcaseBusiness size={17} className="text-[hsl(var(--accent-foreground))]" /> Open application</div><span className="eyebrow text-[hsl(var(--muted-foreground))]">Secure form</span></div>
           <iframe title="SysPro Technologies careers application form" src="https://docs.google.com/forms/d/e/1FAIpQLSeeyDO3MN8V5LgC9Rf8PpYs8zgEjIp6X9pYXM9zg9P-SpDA3Q/viewform?embedded=true" allowFullScreen loading="lazy" className="h-[730px] w-full bg-[hsl(var(--card))]" data-testid="iframe-careers-form" />
        </div>
      </div>
    </div>
  </section>;
...
function Footer() {
  return <footer className="bg-[hsl(var(--foreground))] text-[hsl(var(--background))]"><div className="mx-auto max-w-[1240px] px-5 py-14 sm:px-8"><div className="grid gap-10 border-b border-[hsl(var(--background)/.14)] pb-12 md:grid-cols-[1.3fr_.7fr_.7fr]"><div><Wordmark light /><p className="mt-5 max-w-[310px] text-sm leading-6 text-[hsl(var(--background)/.55)]">Steady technology support for the systems people depend on.</p></div><div><p className="eyebrow mb-4 text-[hsl(var(--primary))]">Navigate</p><div className="flex flex-col items-start gap-3 text-sm text-[hsl(var(--background)/.68)]"><button onClick={() => scrollToId('services')} data-testid="button-footer-services">Services</button><button onClick={() => scrollToId('about')} data-testid="button-footer-about">About</button><button onClick={() => scrollToId('careers')} data-testid="button-footer-careers">Careers</button></div></div><div><p className="eyebrow mb-4 text-[hsl(var(--primary))]">Reach us</p><div className="flex flex-col items-start gap-3 text-sm text-[hsl(var(--background)/.68)]"><a href="mailto:sysprotechnologies@gmail.com" data-testid="link-footer-email">Email SysPro</a><a href="tel:+919831134162" data-testid="link-footer-phone">Call +91 98311 34162</a><span className="flex items-center gap-2"><MapPin size={14} /> Serving India & beyond</span></div></div></div><div className="flex flex-col justify-between gap-5 pt-7 text-xs text-[hsl(var(--background)/.43)] sm:flex-row"><span>© {new Date().getFullYear()} SysPro Technologies. Built for better uptime.</span><div className="flex gap-5"><span>Technology with a human signal.</span><Linkedin size={15} /></div></div></div></footer>;
}
